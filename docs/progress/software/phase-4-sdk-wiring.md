...
**G4 修复（2026-08-12，提交 7897ed6）：** `Config` 加 per-axis PID 字段（`yaw_kp/ki/kd`、`pitch_kp/ki/kd`，默认 0，0..16383）；`DynamixelBackend.connect` 写非 0 值到舵机 Position P/I/D Gain（EEPROM，torque-off 段）；CLI 加 `--yaw-kp`/`--yaw-ki`/`--yaw-kd`/`--pitch-kp`/`--pitch-ki`/`--pitch-kd`。TDD：`test_pid_fields_default_zero_and_validated`（config）+ `test_connect_applies_pid_per_axis`（backend）+ `test_pid_flags_parse_and_override`（cli）。真机验证：yaw 800/5/5 写入、读回一致。
